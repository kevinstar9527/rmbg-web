// ONNX Runtime Web 图片处理模块
// 使用 WASM 后端在浏览器端进行背景移除处理
...
async function preprocessImage(dataUrl, targetWidth, targetHeight) {
...
// 后处理：将模型输出转换为掩码图片
function postprocessMask(outputTensor, originalWidth, originalHeight, scale, offsetX, offsetY) {
...
async function removeBackground(dataUrl, inputSize = 1024) {
    try {
        ...
        const { tensor, originalWidth, originalHeight, scale, offsetX, offsetY } = 
            await preprocessImage(dataUrl, inputSize, inputSize);
        ...
        // 后处理生成掩码
        const maskCanvas = postprocessMask(
            outputTensor, 
            originalWidth, 
            originalHeight, 
            scale, 
